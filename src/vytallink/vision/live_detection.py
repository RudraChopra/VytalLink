...
def main(argv: list[str] | None = None) -> int:
    ...
    failed = 0
    frame_idx = 0
    class_counts: Counter[str] = Counter()  # detected class -> count (aggregate)
    ...
    if detector.mps_fallback_reason:
        print(f"device fallback   : {detector.mps_fallback_reason}  -> now {detector.device_str}")
    print(f"detected classes  : {class_summary}")
    if ch.get("last_error"):
        print(f"last camera error : {ch.get('last_error')}")  # already sanitized
    if frames == 0:
        print("\nRESULT: LIVE_FAIL (no frames — check camera/network/credentials).", file=sys.stderr)
        return 1
    print("\nRESULT: LIVE_OK")
    return 0
